...
<div class="row">
	<div class="col-lg-12">
		<h1 class="page-header">취소완료조회</h1>
	</div>
	<!-- /.col-lg-12 -->
</div>
<!-- /.row -->
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				<table width="100%"
					...
					<tbody>
						<c:choose>
							<c:when test="${not empty cancelOkList }">
								<c:forEach var="cancelOk" items="${cancelOkList }"
									varStatus="status">
									<tr class="odd gradeX" data-num="${cancelOk.r_num }">
										<td>
											<div class="checkbox">
												<label> <input type="checkbox" name="chk_cancel"
													id="chk_cancel" value="${cancelOk.r_num }">
												</label>
											</div>
										</td>
										<td class="goDetail">${cancelOk.r_num }</td>
										<td>${cancelOk.m_id}</td>
										<td>${cancelOk.adminMemberVO.m_name}</td>
										<td>${cancelOk.adminMemberVO.m_phone}</td>
										<td>${cancelOk.r_cancelstatus}</td>
										<td>${cancelOk.c_num}</td>
										<td>${cancelOk.r_prerentaldate}</td>
										<td>${cancelOk.r_prereturndate}</td>
										<td>${cancelOk.r_date}</td>
									</tr>
								</c:forEach>
							</c:when>
							<c:otherwise>
