		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				<table width="100%"
					...
					<thead>
						<tr>
							<th data-value="n_num" class="order">글번호 <c:choose>
									<c:when
										test="${data.order_by=='n_num'and data.order_sc=='ASC'}">▲</c:when>
									<c:when
										test="${data.order_by=='n_num'and data.order_sc=='DESC'}">▼</c:when>
						...
						<c:choose>
							<c:when test="${not empty noticeList }">
								<c:forEach var="notice" items="${noticeList }"
									varStatus="status">
									<tr class="odd gradeX" data-num="${notice.n_num }">
										<td>${notice.n_num }</td>
...
<script type="text/javascript">
	$(function() {
		...
		$(".goDetail").click(function() {
			...
			//상세 페이지로 이동하기 위해 form추가 (id : "noticeDetail")
			$("#noticeDetail").attr({
				"method" : "get",
				"action" : "noticeDetail"
			});
