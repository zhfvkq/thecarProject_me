package com.thecar.common.page;
...
public class PagingTag extends TagSupport {
	...
	public String getPaging() {
		...
		if (prevFirst > 0) {
			ret += "<li> <a href=\"javascript:goPage('" + prevFirst
					+ "');\"><span><img src='/resources/images/common/btn_paginate_prev.gif'  alt='이전' /></span></a> </li>";
		} else {
		...
		for (int j = currentFirst; j < currentFirst + page_size && j <= lastPage; j++) {
			if (j <= currentlast) {
				if (j == page) {
					ret += "<li> <a href='#' class='on textAn'>" + j + "</a> ";
				} else {
		...
		if (nextFirst <= lastPage) {
			ret += "<li> <a href=\"javascript:goPage('" + nextFirst
					+ "')\"><span><img src='/resources/images/common/btn_paginate_next.gif' alt='다음' /></span></a> </li>";
		} else {
		...
		if (page < lastPage) {
			ret += "<li> <a href=\"javascript:goPage('" + lastPage
					+ "')\"><span><img src='/resources/images/common/btn_paginate_last.gif' alt='마지막' /></span></a></li> ";
		} else {
